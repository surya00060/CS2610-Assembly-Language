...
;==================================================
		...
		mov ax,0x28
		mov fs,ax      ;fs starts at 0xb400(R/W)
		mov ax,0x30
...
mat_sub:
	...
	SUB:
		...
		imul ebx, 2 					; ebx = 2*ebx		// Half word: 16bit

		mov ecx, [DS:ebx]				; ecx = A[i][j]
		sub ecx, [ES:ebx]				; ecx = A[i][j] - B[i][j]
		mov [FS:ebx] , ecx 				; C[i][j] = A[i][j] - B[i][j]
		inc esi							; j++
		jmp L2							
...
mat_sq:
	...
	mov eax , [esp+8]
	mov FS,AX   						;FS is source 

	mov edi, 0 				 			; Loop Variable i for loop L1
	mov esi, 0 				 			; Loop Variable j for loop L2
	mov ecx, 0				 			; Sum = 0  To store the squared norm of matrix D

	Loop1:
		cmp edi	, 3						;checks if i < N
		jge END2						; if not goes to end
		mov esi, 0						; else sets j =0 
		jmp Loop2						; Starts loop L2

	Loop2:
		cmp esi, 3						; checks if j < N
		jl SQ							; If So starts the L3
		inc edi							; else i++
		jmp Loop1 						; and Jumps to loop L1; Position (i,j) in 2D Matrix is in position ( C*i + j ) in a 1D Array.
	...
	SQ:
		...
		mov eax, [FS:ebx]				; eax = C[i][j]
		mul eax 						; eax = C[i][j]*C[i][j]
		mov [GS:ebx] , eax 				; D[i][j] = eax

		mul eax 						; eax = D[i][j]*D[i][j]
		add ecx , eax  					; ecx = ecx + D[i][j]^2
		
		inc esi 						; J++
		jmp Loop2			

	END2:
		add ebx , 2  					; Writing the sum squred norm After N^2 elements in Segment GS.
		mov [GS:ebx] , ecx 				; Storing in Base Address + ( N^2 * Size )
		ret
