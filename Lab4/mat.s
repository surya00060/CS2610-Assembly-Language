...
matrix_mul:
	...
	L3:
		mov ebp, 0						; k = 0 
		mov ecx, 3 						; ebx = C2
		imul ecx, edi					; ebx = C2*i
		add ecx, esi   					; ebx = C2*i + j
		imul ecx, 2 					; ebx = 2*ebx

		mov ebx, 0						; ecx = 0
		mov [ES:ecx] , ebx 				; eax = 0
		jmp LO3							

	LO3:
		cmp ebp, 3						; checks if k < C1
	...
	MULT:
		...
		mov edx,[DS:ebx] 				; edx = Base Addres of M1 + 2*ebx
		...
		imul edx,[FS:ebx] 				; edx = Base Address of M3 + ebx  * Base Addres of M2 + 2*ebx
		add [ES:ecx] , edx              ; M3[i][j] += edx
