...
;==================================================
		...
		mov ax,0x20
		mov es,ax      ;es starts at 0xb200(R/W)
		mov ax,0x28
...
matrix_mul:
		pop EBX  ;return address is stored into EBX
		mov ECX,0x0
		mov dword[GS:0x80],EBX ; return address in GS:0x80
		pop EAX
		mov ES,AX   ;ES is destination 
		pop EAX
		mov DS,AX   ;DS is source 1
		pop EAX
		mov FS,AX   ;FS is source 2
		mov ECX,0x0 ; i <- ECX	
		mov ESI,0X0 ; j <- ESI
		mov EDI,0x0 ; k <- EDI

istart: cmp ECX, 0x0A ;i !=10d
		je ifinish
		mov ESI,0x0    ;j <- 0
jstart: 
		cmp ESI,0x0A  ;j !=10d
		je jfinish
		mov EDI,0X0  ;k <- 0
		mov byte [GS:0x100],0x0   ; storing or ed value
kstart: 
		cmp EDI,0x0A  ;k!=10d
		je kfinish
		mov EAX, 0x0A
...
ifinish:
		mov ECX,0x0 ;
		mov dword EBX, [GS:0x80] ;
		push EBX  ;return address is pushed back
		ret
L2:
...
;Segment Descriptor for Segment C' (index 38)
dw 0x0200	;Limit 15->0
dw 0xb800	;Base 15->0
db 0		;Base 23->16
db 10010010b;P=1 DPL=0 S=1 Type=0010(R/W)
